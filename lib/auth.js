 ...
 * Also persists cookies to disk so CLI works without Chrome running.
 */
...
const PROFILE_DIR = path.join(process.env.LOCALAPPDATA || path.join(os.homedir(), 'AppData', 'Local'), 'recorder-cli', 'chrome-profile');
const CONFIG_DIR = path.join(os.homedir(), '.config', 'recorder-cli');
...
// ============================================
function buildAuthHeaders(sapisid, cookieStr, authUser) {
  const au = authUser != null ? authUser : 0;
  return {
    'Authorization': `SAPISIDHASH ${generateSAPISIDHASH(sapisid)}`,
    'X-Goog-Api-Key': API_KEY,
    'X-Goog-AuthUser': String(au),
...
function loadAuthFromFile() {
  try {
    ...
    if (!data.sapisid || !data.cookieStr) return null;
    return {
...
async function getAuth(options = {}) {
  ...
  const authUser = options.authUser || 0;

  // Step 0: Try file-based auth first (no Chrome needed)
  if (!options.forceChrome) {
    const saved = loadAuthFromFile();
    if (saved) {
      log('[AUTH] Found saved credentials. Verifying...');
      const headers = buildAuthHeaders(saved.sapisid, saved.cookieStr, saved.authUser);
      const valid = await testAuth(headers);
      if (valid) {
        log('[AUTH] [OK] Authenticated from saved credentials.');
        return { ...saved, authUser: saved.authUser };
      }
      log('[AUTH] [WARNING] Saved credentials expired.');
    }
  }
  ...
  if (auth) {
    ...
    const headers = buildAuthHeaders(auth.sapisid, auth.cookieStr, authUser);
    const valid = await testAuth(headers);
    if (valid) {
...
// ============================================
// Check auth status (for --check flag)
// ============================================
async function checkAuth(options = {}) {
  ...
  const saved = loadAuthFromFile();
  if (!saved) {
    return { authenticated: false, message: 'No saved credentials found.' };
  }
  ...
  return {
    authenticated: false,
    savedAt: saved.savedAt,
    message: 'Saved credentials have expired. Run auth again.',
  };
}
